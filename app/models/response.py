...
class PageInfo(BaseModel, Generic[T]):
    ...
    pageSize: int = Field(default=10, ge=1, description="每页大小")
    total: int = Field(default=0, ge=0, description="总记录数")
    totalPages: int = Field(default=0, ge=0, description="总页数")
    lists: List[T] = Field(default_factory=list, description="分页数据列表")
    ...
    @classmethod
    def from_list(
        cls, items: List[T], page: int = 1, page_size: int = 10
    ) -> "PageInfo[T]":
        """从列表创建分页信息（假分页）"""
        total = len(items)
...
class R(BaseModel, Generic[T]):
    ...
    class Config:
        from_attributes = True
        json_schema_extra = {"example": {"code": 200, "msg": "操作成功", "data": None}}

    # ==================== 成功响应 ====================

    @classmethod
    def ok(
        cls, data: Optional[T] = None, msg: str = "操作成功", code: int = CodeEnum.OK
    ) -> "R[T]":
        """
        返回成功响应
    ...
    @classmethod
    def ok_page(cls, page_info: PageInfo[T], msg: str = "查询成功") -> "R[PageInfo[T]]":
        """
        返回分页成功响应
    ...
    @classmethod
    def param_error(cls, msg: str = "参数错误", data: Optional[T] = None) -> "R[T]":
        """返回参数错误响应"""
        return cls.fail(msg=msg, data=data, code=CodeEnum.PARAM_ERROR)

    @classmethod
    def not_found(cls, msg: str = "资源不存在", data: Optional[T] = None) -> "R[T]":
        """返回资源不存在响应"""
        return cls.fail(msg=msg, data=data, code=CodeEnum.NOT_FOUND)

    @classmethod
    def unauthorized(cls, msg: str = "未授权", data: Optional[T] = None) -> "R[T]":
        """返回未授权响应"""
        return cls.fail(msg=msg, data=data, code=CodeEnum.UNAUTHORIZED)

    @classmethod
    def forbidden(cls, msg: str = "禁止访问", data: Optional[T] = None) -> "R[T]":
        """返回禁止访问响应"""
    ...
    @classmethod
    def warn(cls, msg: str = "警告", data: Optional[T] = None) -> "R[T]":
        """
        返回警告响应
...
def page_success(
    items: List[T], page: int = 1, page_size: int = 10, msg: str = "查询成功"
) -> R[PageInfo[T]]:
    """
    快速构建分页成功响应
